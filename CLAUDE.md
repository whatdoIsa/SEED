...
- **인프라**: iCloud 백업(복원 병합 refreshAfterRemoteImport), 홈/잠금 위젯(App Group+딥링크 seed://daily), 로컬 알림 4종(아침 08시 루틴·저녁 20시 오늘의장 리마인더[7일 개별 예약, 완료 시 당일 취소]·일 19시 주간 복기·**체결 영수증**[시장 탭+ETF의 사용자 체결 즉시 "🧾 한빛전자 100주 매수 체결 · 주당/총액", 게임 모드는 제외, 포그라운드 배너는 SeedNotificationDelegate] — 기본 전부 켬, 설정 토글), 설정 화면, 앱아이콘/런치(런치 화면=아이콘 마크, RootView 오버레이가 픽셀 동일한 한 장을 0.25s 유지 후 0.35s 페이드 — 애니메이션 시퀀스 없음, 진입 지연 최소화가 사용자 결정), 접근성 요약, 하루1레슨 페이스
